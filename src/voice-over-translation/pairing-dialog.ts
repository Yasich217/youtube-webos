...
function appendNodes(parent: Node, ...children: Array<Node | string>): void {
...
function replaceNodes(parent: Node, ...children: Array<Node | string>): void {
...
#${DIALOG_ID} .ytaf-vot-pair-button + .ytaf-vot-pair-button {
  margin-left: 18px;
}
...
function ensureStyles(): void {
  ...
  appendNodes(document.head ?? document.documentElement, style);
}
...
function countdownText(expiresAt: number | null): string {
  ...
  const seconds = secondsValue < 10 ? `0${secondsValue}` : String(secondsValue);
...
class PairingDialog {
  ...
  constructor() {
    ...
    appendNodes(this.#root, this.#card);
    ...
    appendNodes(document.body ?? document.documentElement, this.#root);
  }
  ...
  #renderFrame(
    ...
    appendNodes(contentContainer, content);
    const actions = createElement('div', 'ytaf-vot-pair-actions');
    appendNodes(actions, ...buttons);
    const hint = createElement(
      'div',
      'ytaf-vot-pair-hint',
      'Use arrow keys to move · OK to select · Back to cancel'
    );
    replaceNodes(this.#card, heading, contentContainer, actions, hint);
  ...
  #renderPairing(session: PairingSession): boolean {
    ...
    appendNodes(qrContainer, qrCode);
    ...
    appendNodes(copy, countdown);
    appendNodes(columns, qrContainer, copy);
