...
- Centered the Yandex pairing dialog on Chromium 53 using explicit viewport
  edges and a transform fallback instead of unsupported `inset` and `min()`.
- Replaced flex `gap` with legacy-compatible sibling margins and added bounded
  1080p overflow handling.

## [0.8.5] - 2026/08/15

### Voice-over Translation

- Fixed language-change debounce on Chromium 53 by preserving the required
  receiver semantics of legacy Window timer functions.
- Added a receiver-sensitive regression test matching the C9 browser runtime.

## [0.8.4] - 2026/08/15

> Superseded by `0.8.5-vot.3`: legacy Chromium could reject the new language
> debounce timer before its final translation request.
