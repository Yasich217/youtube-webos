...
| Installed app   | `youtube.leanback.v4` downstream `0.8.6` (`vot.4`, based on upstream `0.8.2`)       |
...
```sh
ares-install -d c4 ./youtube.leanback.v4_0.8.6_all.ipk
...
nvm exec 20 node node_modules/@webos-tools/cli/bin/ares-install.js \
  -d c4 ./youtube.leanback.v4_0.8.6_all.ipk
```
...
public HTTPS `repo.json` after the matching `0.8.6` IPK has been published,
...
`vot.4+upstream.0.8.2` separately. Do not add an unpublished or private URL.
