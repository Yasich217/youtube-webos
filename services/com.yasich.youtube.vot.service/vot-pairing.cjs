...
function createLoopbackServer(manager) {
  const server = http.createServer(async (req, res) => {
    ...
    const remoteAddress = String(req.socket.remoteAddress || '').split('%')[0];
    ...
    const hasExactLoopbackHost =
      Number.isSafeInteger(localPort) &&
      localPort > 0 &&
      host === '127.0.0.1:' + localPort;
    if (!isLoopback || !hasExactLoopbackHost) {
    ...
    // Chromium 53's privileged WAM omits Origin only on the loopback status
    // GET. Keep that compatibility exception exact: POST control requests
    // still need the YouTube/null origin, and the temporary LAN /v1/pair
    // listener retains its separate strict Host/Origin/secret checks.
    const isLegacyStatusGet =
      !origin && req.method === 'GET' && req.url === '/v1/auth/status';
    if (!isLegacyStatusGet && !ALLOWED_ORIGINS.has(origin)) {
